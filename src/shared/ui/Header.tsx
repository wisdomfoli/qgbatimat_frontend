        ...
        <div className="max-w-7xl mx-auto px-4 h-16 md:h-20 flex items-center gap-4 lg:gap-8">
                    ...
                    {categories.map((cat) => (
                      ...
                      >
                        {cat.icon ? (
                          ...
                          <span className="w-1.5 h-1.5 rounded-full bg-[#e8541a] ml-2 mr-1" />
                        )}
          ...
          <form onSubmit={handleSearch} className="hidden md:block flex-1 min-w-0 max-w-xl mx-auto">
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-10 rounded-full bg-zinc-100 border border-transparent pl-11 pr-10 text-sm text-zinc-700 placeholder-zinc-400 outline-none focus:border-[#e8541a] focus:bg-white transition-all"
              />
              {searchQuery && (
                <button
                  type="button"
          ...
          <div className="flex items-center gap-1 shrink-0 ml-auto md:ml-0">
            {isAuthenticated ? (
              <Link
                to="/profile"
                className="hidden md:inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-zinc-700 hover:bg-zinc-100 rounded-full transition-colors"
              >
              ...
              <form onSubmit={handleSearch} className="mb-5">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full h-10 rounded-full bg-zinc-100 pl-10 pr-10 text-sm text-zinc-700 outline-none focus:bg-white border border-transparent focus:border-[#e8541a] transition-all"
                  />
                  {searchQuery && (
                    <button
                      type="button"
              ...
              <div className="flex flex-col gap-2 mb-6">
                {isAuthenticated ? (
                  <Link
                    to="/profile"
                    onClick={() => setMobileOpen(false)}
                    className="flex items-center justify-center gap-2 px-4 py-2.5 bg-gray-100 text-sm font-medium text-zinc-800 rounded-xl transition-colors"
                  >
                ...
                {categories.map((cat) => (
                  ...
                  >
                    {cat.icon ? (
                      ...
                      <span className="w-1.5 h-1.5 rounded-full bg-[#e8541a] ml-1 mr-2" />
                    )}
                    <span className="flex-1">{cat.name}</span>
                  </Link>
                ))}
