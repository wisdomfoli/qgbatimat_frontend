    ...
    <div className="group flex flex-col bg-white shadow-sm rounded-2xl overflow-hidden p-4">
      ...
      <div className="relative overflow-hidden rounded-2xl md:rounded-3xl bg-zinc-100 aspect-square">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5 pointer-events-none z-10">
          {product.badge && (
            <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold leading-none ${BADGE_STYLES[product.badge]}`}>
        ...
        <div className="flex items-center gap-1 mt-1.5">
          {[1, 2, 3, 4, 5].map((star) => (
            <Star
              key={star}
              size={11}
              className={star <= Math.round(product.rating) ? 'fill-[#e8541a] text-[#e8541a]' : 'fill-zinc-200 text-zinc-200'}
            />
          ))}
