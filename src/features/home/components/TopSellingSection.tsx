...
export default function TopSellingSection() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const data = await productService.getProducts({ top_selling: 1, limit: 4 })
        setProducts(data)
      } catch (err) {
    ...
    <section className="pb-6 md:pb-6 mb-6">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <p className="text-sm text-zinc-500 text-center mb-8">
          Les produits les plus commandés par les professionnels du bâtiment
        </p>
        ...
        {bestsellers.length > 0 ? (
          ...
          <div className="py-12 border-2 border-dashed border-zinc-100 rounded-3xl flex flex-col items-center justify-center text-center">
            <p className="text-zinc-400 font-medium italic">Pas encore de produits dans le top des ventes.</p>
          </div>
        )}
      </div>
    </section>
